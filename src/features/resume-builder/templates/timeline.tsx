import type { ResumeData, ResumeStyle, SectionKind } from "../types";
...
export function TimelineTemplate({ data, style }: { data: ResumeData; style: ResumeStyle }) {
  const sections = orderedSections(data).filter((s) => s.hasContent);

  return (
    <ResumePage style={style} className="px-10 py-9">
      ...
      {sections.map((s) =>
        TIMELINE_KINDS.has(s.kind) ? (
          ...
          <SectionBody key={s.kind} kind={s.kind} data={data} look={plainLook} />
        ),
      )}
    </ResumePage>
  );
